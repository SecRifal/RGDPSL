document.addEventListener('DOMContentLoaded', () => {
    ...
    // Загрузка данных
    fetch('https://raw.githubusercontent.com/SecRifal/RGDPSJ/main/memory.json')
        .then(response => response.json())
        .then(data => {
            memoryData = data.sort((a, b) => b.stars - a.stars); // Сортировка по stars убывающей
            renderLevels(memoryData);
        })
    ...
    function renderLevels(levels) {
        ...
        levels.forEach((level, index) => {
            const position = memoryData.indexOf(level) + 1;
            ...
            let difficultyHTML = '';
            if (level.diff) {
                const diffIconPath = `../../imgs/diff/${level.diff.toLowerCase().replace(' ', '-')}.png`;
                difficultyHTML = `<img src="${diffIconPath}" alt="${level.diff}" class="difficulty-icon" onerror="this.style.display='none'">`;
            }

            levelElement.innerHTML = `
                <div class="position">${position}</div>
                <img src="https://raw.githubusercontent.com/SecRifal/RGDPSJ/main/images/memory/${level.name}.png" alt="${level.name}" class="level-image" onerror="this.src='https://raw.githubusercontent.com/SecRifal/RGDPSJ/main/images/placeholder.png'">
    ...
    function openModal(level, position) {
        ...
        modalBody.innerHTML = `
            ...
            <img src="https://raw.githubusercontent.com/SecRifal/RGDPSJ/main/images/memory/${level.name}.png" alt="${level.name}" class="level-modal-image" onerror="this.src='https://raw.githubusercontent.com/SecRifal/RGDPSJ/main/images/placeholder.png'">
    ...
    function showVictors(levelID) {
        const level = memoryData.find(m => m.levelID.toString() === levelID.toString());
    ...
    searchInput.addEventListener('input', (e) => {
        ...
        const filteredLevels = memoryData.filter(level =>
            level.name.toLowerCase().includes(query) ||
            level.levelID.toString().includes(query) ||
            memoryData.indexOf(level).toString().includes(query) ||
    ...
    function applyTextGlowEffect(container) {
        ...
        textElements.forEach(element => {
            if (element.querySelector('a') || element.querySelector('img') || element.querySelector('.copy-id')) return;
